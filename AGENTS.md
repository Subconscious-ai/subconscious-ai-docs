...
- **OpenAPI or MCP provenance**: regenerate content in the owning repository,
  then synchronize the verified release and every revision/digest in
  `site/provenance/sources.json` together. A green source PR does not prove its
  artifact is adopted on the default branch.
